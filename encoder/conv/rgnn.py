...
class RGNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_type, edge_weight = None):
        ...
        #gnn aggregation
        if edge_weight == None:
            x = self.layer1(x, edge_index, edge_type).relu()
            for layer in self.layers:
                x = layer(x, edge_index, edge_type).relu()
                x = self.dropout(x)  
        else:
            x = self.layer1(x, edge_index, edge_type, edge_weight).relu()
            for layer in self.layers:
                x = layer(x, edge_index, edge_type, edge_weight).relu()
                x = self.dropout(x)   
